// src/middleware.ts (Versi Upgrade)
import { defineMiddleware } from "astro:middleware";

interface Post {
  published: string;
  // Anda bisa tambahkan properti lain di sini jika dibutuhkan di masa depan
  // id: string;
  // title: string;
}

async function fetchBlogInfo(BLOG_ID: string, API_KEY: string) {
...
export const onRequest = defineMiddleware(async (context, next) => {
  if (context.locals.blogData && context.locals.recentPosts) {
    return next();
  }
  ...
  if (!BLOG_ID || !API_KEY) {
    ...
    context.locals.blogData = { name: "Bima Akbar", description: "Default description" };
