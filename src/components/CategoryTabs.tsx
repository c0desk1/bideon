  ...
  return (
    <div>
      <div className="flex flex-wrap justify-left gap-2 mb-6">
        <button
          className={`py-2 px-4 rounded-full cursor-pointer ${activeCategory === 'Terbaru' ? 'bg-blue-600 text-white' : 'bg-zinc-700 text-white'}`}
        ...
        {categories.map((cat) => (
          <button
            key={cat}
            className={`py-2 px-4 rounded-full cursor-pointer ${activeCategory === cat ? 'bg-blue-600 text-white' : 'bg-zinc-700 text-white'}`}
      ...
      {activeCategory !== 'Terbaru' && posts.length >= 3 && (
        <div className="text-center mt-6">
          <a
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md text-sm font-medium text-white hover:border-white/20 hover:bg-white/10 transition-colors duration-200 cursor-pointer"
          >
